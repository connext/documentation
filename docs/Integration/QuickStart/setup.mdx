...
2. connext SDK ที่รันอยู่บนบล็อคเชนแยก (offchain)
...
SDK นั้นใช้งานในรูปแบบของ typescript package บน npm และตัว SDK นั้น isomorphic อย่างสมบูรณ์​ (สามารถใช้ได้เหมือนกันไม่ว่าจะเป็น ฝั่งผู้ใช้ (client) ฝั่งเซิร์ฟเวอร์ (server) และ React Native environments)
...
- `config`: _จำเป็น_. Object ที่มี chainConfig, ผู้เซ็น (signer) and parameters อื่นๆ.
  - `chainConfig`: _จำเป็น_. Object ที่มี `chainId` เป็น key, และ object ที่มีค่า keys/values ตามนี้ ให้ใส่บล็อคเชนที่ผู้ใช้งานต้องการจะรอบรับใน argument นี้:
    - `provider`: _จำเป็น_. [FallbackProvider](https://docs.ethers.io/v5/api/providers/other/#FallbackProvider) ของ Ethers. Providers จะถูกเริ่มต้นด้วย [Alchemy](https://www.alchemy.com), [Infura](https://infura.io), หรือ local nodes. `FallbackProvider` จะถูกใช้งานเพื่อรองรับกรณีที่ provider นั้นผิดพลาด
