...
Follow these instructions to install [Node.js](https://nodejs.dev/en/learn/how-to-install-nodejs/) and use **Node.js v16**. We also recommend installing `nvm`, a node version manager, which will make switching versions easier.
...
```
Next, install the OpenZepplin's contract package:
...
const main = async () => {
  ...
  const sourceGreetingContract = await SourceGreeting.deploy("0x99A784d082476E551E5fc918ce3d849f2b8e89B6");
...
Follow the same steps above to create a new hardhat project, this time name the project `GreetingDestination.` Install all the same dependencies as above. 
...
- `amount`: 10000000000000000000 (10 TEST)
  - Recall that `HelloTarget` requires a payment of at least 1 TEST (1e18 wei units). In `SourceGreeting`, this is hardcoded as 1000500300000000000 to account for a 0.05% fee that routers will take on the bridged asset. But you can approve as much as you want. This way, you can call `updateGreeting` without having to do an approval every time.
