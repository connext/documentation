...
In this quickstart, there are two contracts. The `HelloTarget` contract on the destination domain (Mumbai) stores a greeting that we want to update. The `HelloSource` contract on the origin domain (Goerli) will call `xcall` to execute the `_updateGreeting` function on `HelloTarget`.

### Target Contract

The target contract must implement Connext's `IXReceiver` interface. This interface ensures that Connext can call the contract and pass necessary data. 

It will also require that the originating call includes a payment of at least 1 TEST to update the greeting.
