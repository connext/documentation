...
`updateGreeting` is implemented as an unauthenticated call (there are no checks to determine *who* is calling the function). Therefore, this type of `xcall` will be go through the ["Fast Path"](../concepts/how-it-works/flow).
